(define (domain factorio)
    ...
    (:action create-ysplitter
        ...
        :precondition( and
            (existing-plate ?plate-0)
            (existing-plate ?plate-1)
            (existing-circuit ?circuit-0)
            (existing-ybelt ?ybelt-0)
            (possible-ysplitter ?ysplitter)
        )
        :effect (and
            (consumed-plate ?plate-0)
            (consumed-plate ?plate-1)
            (consumed-circuit ?circuit-0)
            (consumed-ybelt ?ybelt-0)
            (existing-ysplitter ?ysplitter)
            (not(existing-plate ?plate-0))
            (not(existing-plate ?plate-1))
            (not(existing-circuit ?circuit-0))
            (not(existing-ybelt ?ybelt-0))
